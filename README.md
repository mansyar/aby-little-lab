...
An ad-free, distraction-free developmental game suite for preschoolers aged 3-5. Thirteen mini-games targeting cognitive, motor, literacy, numeracy, and reasoning milestones, built with Phaser 4 + TypeScript + Vite as an installable PWA.
...
The protected controls (Hub Settings, all fifteen game Back buttons) and the shared replay speaker button (Find the Letter, Find the Word, Build the Word, How Many?, First Sounds, More or Less, Odd One Out, Color Match, and Musical Memory — where it replaced the emoji replay) expose explicit **96×96px hit areas**, so children can tap near the visible control without precision aiming. Phaser hit areas are anchored at the top-left of a control's bounds, not its origin — keep `Rectangle(0, 0, 96, 96)` even for right-aligned controls; the centered speaker icon uses `Rectangle(-48, -48, 96, 96)`.
...
- **Win celebration** — all fifteen games share one choreographed completion effect: 10 rays + 10 drifting confetti bits (~700ms) that clean themselves up and never block the next interaction; the first-time sticker reveal pops ~400ms later so the confetti clears first.
...
- **Tile icons** — every tile shows a distinct storybook icon (80px, `src/assets/svg/ui/tiles/`) above a small text label, so the 15 games are identifiable at a glance without reading (2026-08-06).
...
The fifteen game scenes are **lazy-loaded** so the startup bundle only contains the app shell (Phaser + Boot/Preload/Hub):
...
- **Build output:** a single entry chunk (~1.44 MB / 372 KB gzip — Phaser dominates the shell) plus fifteen 3–5 KB game-scene chunks that are fetched only on the first tap of each game. Rollup auto-hoists shared modules (game logic, `dragJuice`, `completionEffect`).
