...
An ad-free, distraction-free developmental game suite for preschoolers aged 3-5. Twenty mini-games targeting cognitive, motor, literacy, numeracy, and reasoning milestones, built with Phaser 4 + TypeScript + Vite as an installable PWA.
...
├── scenes/                # BootScene, PreloadScene, HubScene (shell) + 20 lazy-loaded game scenes + sceneRegistry (dynamic-import loaders)
...
The protected controls (Hub Settings, all twenty game Back buttons) and the shared replay speaker button (Find the Letter, Find the Word, Build the Word, How Many?, First Sounds, More or Less, Odd One Out, Color Match, and Musical Memory — where it replaced the emoji replay) expose explicit **96×96px hit areas**, so children can tap near the visible control without precision aiming. Phaser hit areas are anchored at the top-left of a control's bounds, not its origin — keep `Rectangle(0, 0, 96, 96)` even for right-aligned controls; the centered speaker icon uses `Rectangle(-48, -48, 96, 96)`.
...
- **Win celebration** — all twenty games share one choreographed completion effect: 10 rays + 10 drifting confetti bits (~700ms) that clean themselves up and never block the next interaction; the first-time sticker reveal pops ~400ms later so the confetti clears first.
...
- **Memory Match** — a face-down grid deals in with a staggered pop (40ms per card), and each tap flips a card with a scaleX 1→0→1 animation and a soft pop. Matching a pair flashes both cards green with the correct chime and a Hoot cheer; a mismatch wiggles both cards (±4°, 3 yoyo repeats) with a gentle mascot nod, then flips them back after a short pause — no penalty. Cards grow smaller and the grid grows (2×3 → 3×4 → 4×4) across the six rounds as the child's working memory warms up.
...
- **Tile icons** — every tile shows a distinct storybook icon (80px, `src/assets/svg/ui/tiles/`) above a small text label, so the 18 games are identifiable at a glance without reading (2026-08-06).
...
The twenty game scenes are **lazy-loaded** so the startup bundle only contains the app shell (Phaser + Boot/Preload/Hub):
...
- **Build output:** a single entry chunk (~1.44 MB / 372 KB gzip — Phaser dominates the shell) plus twenty 3–5 KB game-scene chunks that are fetched only on the first tap of each game. Rollup auto-hoists shared modules (game logic, `dragJuice`, `completionEffect`).
