...
- **Adaptive Literacy & Memory** *(2026-08-27)*: The same adaptive ±1 band system extends to six literacy & memory games — Find the Word, Build the Word, First Sounds, Find the Letter, Memory Match, and Musical Memory — re-evaluated fresh at each playthrough start from the kid's last 10 taps in that game (per profile, never mid-game, invisible to kids). Struggling kids get easier runs (all-3-letter words, B/P-free phonics targets, A–J letter targets, smaller memory grids, 1-note starting sequences); thriving kids get stretched runs (more 4-letter words, confusable-distractor discrimination practice, bigger grids, longer frog sequences); with too few taps or the toggle off, the classic ladder is used. Same Settings → Adaptive Difficulty toggle; no new controls.
- **Replay Variety:** Items/shapes/animals shuffle per playthrough; difficulty stays fixed. *(Amended 2026-08-27: in the four early-numeracy games and the six literacy & memory games — Find the Word, Build the Word, First Sounds, Find the Letter, Memory Match, Musical Memory — difficulty may now shift ±1 band per playthrough via Adaptive Difficulty above; all other games remain fixed-difficulty.)*
