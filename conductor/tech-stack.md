...
- **Scenes:** 22 scenes (Boot, Preload, Hub, 19 game scenes)
...
└── src/
    ...
    │   ├── PreloadScene.ts
    │   ├── HubScene.ts
    │   ├── GameSceneBase.ts         # Shared scaffold for all 19 game scenes (back button, mascot, progress dots, win/sticker/auto-return)
    │   ├── ShapeSorterScene.ts
    │   ├── AnimalTraceScene.ts
    │   ├── PopFreezeScene.ts
    │   ├── ShadowMatchScene.ts
