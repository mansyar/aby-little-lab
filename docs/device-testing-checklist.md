...
- **Checks:** Settings opens via the 3s parental hold; Progress page 1 shows 6 rows (Shape Sorter … Big & Small); page 2 shows 6 rows (Pattern Builder … First Sounds); page 3 shows 5 rows (More or Less … Take Away); no text collisions anywhere — row stats vs next row's game name, first row vs profile chip, last row vs "N / 3" footer, stats vs accuracy bars; More/Back wrap-around works; footer shows `v1.14.1`; glyph font consistency — Find the Letter / First Sounds / How Many? / Build the Word letters and numerals render in the rounded bundled Baloo 2 style, matching the app's UI text on every device class (glyph font fix, track baloo2-glyphs_20260811 — letterforms identical across devices, no Arial fallback visible)
...
- **Result:** reference-profile executed 2026-08-09 (desktop Chromium, playwright-cli) — boot 1080.7 ms (< 3 s), shell + phaser chunks confirmed as separate requests, no console errors beyond pre-existing favicon 404; **device-class execution completed by user 2026-08-09 against the live URL — all items passed, no issues found**
- **Issues found:** none on reference profile (no Critical/High/Medium/Low)

## Execution Record — v1.14.0 (2026-08-10)

- **Target:** live URL `https://aby-little-lab.ansyar-world.top/` (v1.14.0 — Game 17 Take Away, complete 17-game suite)
- **Scope:** Game 17 (Take Away) rows below, plus carried v1.13.0 rows (Color Match, Add It Up, Perf/Bundle spot-check, Parent Progress Insights — Progress shipped in v1.13.0)
- **Devices:** iPad (iPadOS 15+), Android tablet (Android 10+), iPhone (iOS 15+), Android phone (Android 10+)
- **Checks:** Hub shows 17 tiles in a 5×3 + 2 grid (row 4 holds 2 left-aligned tiles — Add It Up + Take Away — no clipping, tile_take_away icon distinct); Take Away loads from its lazy chunk with no spoken prompt (textless, no speaker button — fully playable without audio); 6 rounds per play; each round shows 2 prompt cards — the minuend group and the subtrahend group (always two different item types) joined by the take-away cue — and exactly 4 answer cards showing the possible differences as dot groups; easy-first bands — rounds 1–2 minuends ≤4, rounds 3–4 ≤6, rounds 5–6 ≤10; differences never reach 0 and no subtraction pair repeats within a playthrough; correct tap → chime + green card flash + mascot cheer + progress dot pop → next round in ~0.7s; wrong tap → gentle wiggle ±4° + mascot nod, no penalty, no progression loss; win after round 6 → confetti + rays + sticker (first completion only, `take-away`) + 3s auto-return with just-earned highlight; replay shows sticker already earned with no re-award; Learning Progress records Take Away plays/wins/accuracy per profile (report now covers all 17 games — rows page 8 + 8 + 1); old v1.13.0 saves load cleanly with no destructive migration; PWA update flow — when a new build is deployed the update prompt appears and applying it serves the new version; offline relaunch — after a full online load, switching to airplane mode and relaunching still boots the game from the service-worker precache
- **Result:** **all items passed on all 4 device classes** — iPad (iPadOS 15+), Android tablet (Android 10+), iPhone (iOS 15+), Android phone (Android 10+) — executed 2026-08-10 by the user against the live URL: Take Away six easy-first rounds (incl. wrong-answer no-penalty), win celebration + first-completion sticker + 3s auto-return + replay without duplicate award, Learning Progress report showing all 17 games (pages 8 + 8 + 1, incl. Take Away with recorded stats), PWA install/update flow, offline relaunch from service-worker precache, old v1.13.0 save migration, per-profile sticker/progress persistence — all passed, no issues found
- **Issues found:** none
- **Automated smoke (2026-08-10, desktop Chromium via playwright-cli against the live URL):** boot to Hub with 17 tiles (5×3+2, no clipping); Take Away full six-round playthrough with correct answers — each round advanced (~0.7s), win celebration + sticker badge shown, 3s auto-return to Hub; storage assertions — sticker `take-away` earned once (earnedAt 2026-08-10T02:42:14Z), progress plays/wins/correct/wrong recorded (3 plays · 100% · Today displayed in the report); wrong-tap gentle-wiggle no-penalty verified; Settings panel footer `v1.14.0`; console clean (0 errors, 0 warnings)
