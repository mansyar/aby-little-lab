...
|---|---|---|---|
| `mascot_idle.svg` | 512×512 | Hub + all seventeen games | Professor Hoot neutral pose (idle bob + blink loop on Hub) |
| `mascot_celebrate.svg` | 512×512 | Hub + all seventeen games | Arms-up cheer pose (bounce + sparkle ring via Graphics) |
...
- **Polish:** sticker tween delayed 400ms/250ms in all seventeen scenes; idle attract wiggles 2 rotating tiles; Preload brand lockup; Shadow Match objects 112px; `sleep_zzz` glyph; footer reposition.
...
- `createWinCelebration(scene, x, y)` — the shared completion effect used by all seventeen games (replaces per-game bespoke win tweens):
...
| Sticker pops (all seventeen games) | 300 ms | 180 ms |
...
- **Scene wiring:** Hub waves on load, cheers on `justEarned`; all seventeen games cheer on correct actions (beside `playCorrect`/`playPop`/round-success), nod on incorrect (`playIncorrect`/`playWake`; Animal Trace has no nod — it's a no-fail game), big cheer on win (beside `playWin`), and destroy on shutdown. Shape Sorter's nod is gated to zone drops, matching the silent-bounce rule.
