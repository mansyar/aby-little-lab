 ...
 */
export class FirstSoundsScene extends Phaser.Scene {
  ...
  private handleComplete(): void {
    ...
    const earnedNow = !hasSticker("first-sounds");
    if (earnedNow) {
    ...
    this.time.delayedCall(AUTO_RETURN_DELAY, () => {
      transitionToScene(this, "Hub", earnedNow ? { justEarned: "first-sounds" } : undefined);
    });
  }
  ...
  private createStickerAnimation(): void {
    const stickerImage = this.add
      .image(this.cameras.main.centerX, this.cameras.main.centerY, "sticker_first_sounds")
      .setScale(0);
