...
/** Mock the TTS wrapper so tests can verify odd-item speech without real voices. */
...
import type { OddOneRound } from "../../game/oddOneOutLogic";
import { promptFor } from "../../game/oddOneOutLogic";
...
describe("OddOneOutScene round flow", () => {
  ...
  function getCurrentRound(scene: unknown): OddOneRound {
    const s = scene as {
      rounds: OddOneRound[];
      roundIndex: number;
    };
  ...
  function getItemImagesByTexture(scene: unknown): Map<string, Array<Record<string, MockFn>>> {
    ...
    for (let i = 0; i < imageMock.mock.calls.length; i++) {
      ...
      if (typeof key !== "string" || key === "mascot_idle") {
        continue;
      }
  ...
  /** Taps the odd card and fires the next-round delay for the current round. */
  function completeRound(scene: unknown): void {
    const round = getCurrentRound(scene);
    tapCard(scene, round.oddSlot);
  ...
  it("renders a 2x2 grid of 4 cards (3 identical + 1 odd) and 6 progress dots", () => {
    const scene = new OddOneOutScene();
    scene.create();
    ...
    // Exactly 3 identical group cards + 1 odd card.
    const itemsByTexture = getItemImagesByTexture(scene);
    expect(itemsByTexture.get(round.groupTexture)).toHaveLength(3);
    expect(itemsByTexture.get(round.oddTexture)).toHaveLength(1);

    expect(getProgressDots(scene)).toHaveLength(6);
  });

  it("speaks the odd item's name at round start when SFX is enabled", () => {
    const scene = new OddOneOutScene();
    scene.create();

    const round = getCurrentRound(scene);
    expect(mockSpeech.speakWord).toHaveBeenCalledWith(promptFor(round.oddTexture), true);
  });
  ...
  it("silences TTS when the SFX toggle is off", () => {
    ...
    const round = getCurrentRound(scene);
    expect(mockSpeech.speakWord).toHaveBeenCalledWith(promptFor(round.oddTexture), false);
  });

  it("tapping the odd card plays the correct chime, flashes success, and advances", () => {
    const scene = new OddOneOutScene();
    scene.create();
    const round = getCurrentRound(scene);
    ...
    expect(getMockFn(cards[round.oddSlot].setFillStyle)).toHaveBeenCalledWith(0x68d391, 1);
    ...
    // The new round re-renders and speaks its own odd item.
    const secondRound = getCurrentRound(scene);
    expect(mockSpeech.speakWord).toHaveBeenCalledWith(promptFor(secondRound.oddTexture), true);
  ...
  it("tapping a group card wiggles gently and does not advance the round", () => {
    const scene = new OddOneOutScene();
    scene.create();
    const round = getCurrentRound(scene);
    const wrongIndex = round.oddSlot === 0 ? 1 : 0;
    const rect = getCardRects(scene)[wrongIndex];
    const wrongItems = getItemImagesByTexture(scene).get(round.groupTexture) ?? [];
    ...
    // The card and one of its group item copies wiggle together.
    const wiggleTween = getMockFn(scene.tweens.add).mock.calls.find((call) => {
      const targets = call[0]?.targets;
      if (!Array.isArray(targets)) return false;
      return targets.includes(rect) && targets.some((t) => wrongItems.includes(t));
    });
  ...
  it("wins after 6 correct rounds: celebration, first-time sticker, justEarned, auto-return", () => {
    ...
    ).mock.calls.find((call) => call[2] === "sticker_odd_one_out");
  ...
  it("generates a valid playthrough for the scene (6 rounds, 3 identical + 1 odd each)", () => {
    const scene = new OddOneOutScene();
    scene.create();
    const rounds = (scene as { rounds: OddOneRound[] }).rounds;
...
describe("OddOneOutScene press feedback cohesion", () => {
  ...
  it("gives every option card press feedback restoring on release, out, and cancel", () => {
    stubMotion(false);
    const scene = new OddOneOutScene();
    scene.create();
  ...
  it("attaches exactly one extra pointerdown listener per card when motion is allowed", () => {
    stubMotion(true);
    const reducedScene = new OddOneOutScene();
    reducedScene.create();
