...
describe("TakeAwayScene round flow", () => {
  ...
  function getCurrentRound(scene: unknown): TakeAwayRound {
    const s = scene as {
  ...
  /** Returns the 2 prompt card rectangles (created without interactivity). */
  function getPromptCardRects(scene: unknown): Array<Record<string, MockFn>> {
    const s = scene as { add: Record<string, unknown> };
    const rectangleMock = getMockFn(s.add.rectangle);
    const prompts: Array<Record<string, MockFn>> = [];
   ...
   * first the two prompt dot-groups, then the four answer dot-groups.
   */
  ...
  /** Taps the answer card matching the target difference and fires the next-round delay. */
  ...
  it("renders 2 prompt cards, minus/equals cues, 4 interactive answer cards, and 6 progress dots", () => {
    const scene = new TakeAwayScene();
    scene.create();

    const round = getCurrentRound(scene);

    // Two prompt cards, non-interactive, with thick outlines.
    const prompts = getPromptCardRects(scene);
    ...
    // The minus and equals cues are present with a pop-in tween.
    const symbols = getSymbolImages(scene);
    expect(symbols.get("minus")).toBeDefined();
    ...
    // Item images in creation order: minuend (count a), subtrahend (count b),
    // then the four answer dot-groups with the round's answer item texture.
    const items = getItemsInOrder(scene);
    const a = round.promptCards[0];
    const b = round.promptCards[1];
  ...
  it("tapping the matching difference card plays the correct chime, flashes success, and advances", () => {
    const scene = new TakeAwayScene();
    scene.create();
    const round = getCurrentRound(scene);
    const targetSlot = round.answerOptions.indexOf(round.target);
  ...
  it("tapping a non-matching card wiggles gently and does not advance the round", () => {
    ...
    const items = getItemsInOrder(scene);
    const aCount = round.promptCards[0].count + round.promptCards[1].count;
  ...
  it("wins after 6 correct rounds: celebration, first-time sticker, justEarned, auto-return", () => {
    ...
    ).mock.calls.find((call) => call[2] === "sticker_take_away");
  ...
  it("generates a valid playthrough for the scene (6 rounds, distinct totals, target present)", () => {
    const scene = new TakeAwayScene();
    scene.create();
    const rounds = (scene as { rounds: TakeAwayRound[] }).rounds;
